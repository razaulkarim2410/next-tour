...
export default async function DetailPage({ params }) {
  ...
  if (!data) {
    return <div className="w-10/12 mx-auto my-10 text-center text-red-500">❌ Product not found</div>;
  }
    ...
    <div className="w-11/12 lg:w-9/12 mx-auto my-16 grid grid-cols-1 md:grid-cols-2 gap-10">
      ...
      <div className="flex flex-col justify-between bg-white rounded-2xl shadow-lg p-8">
        <div>
          ...
          {/* Price & Stock */}
          <div className="mt-6">
            <p className="text-2xl font-bold text-orange-500">💲{data.price}</p>
            <p className={`mt-2 ${data.stock > 0 ? 'text-green-600' : 'text-red-600'}`}>
          ...
          {/* Brand & Category */}
          <div className="mt-6">
            <p className="text-gray-700"><span className="font-semibold">Brand:</span> {data.brand}</p>
            <p className="text-gray-700"><span className="font-semibold">Category:</span> {data.category}</p>
          </div>

          {/* Rating */}
          <div className="flex items-center mt-6">
            <span className="text-yellow-500 text-xl">⭐</span>
            <span className="ml-2 text-gray-800 font-medium">{data.rating} / 5</span>
          </div>
        </div>
